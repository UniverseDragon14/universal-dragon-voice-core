#!/usr/bin/env python3
"""
UNIVERSAL DRAGON - Voice Core for Nova-Pi
Single-file Flask app: browser voice UI (Web Speech ASR) + LLM brain router
+ edge-tts voice output + safe local tools.

Run:
    python3 dragon_voice.py
Env:
    DRAGON_PORT       (default 5066)
    DRAGON_TOKEN      (login token for the web UI; empty = open, not recommended)
    DRAGON_VOICE      (edge-tts voice, default en-US-AndrewNeural)
    DRAGON_WORKSPACE  (file sandbox, default ~/dragon_workspace)
    DRAGON_VOICE_TOKEN    (local voice-soul v2 server bearer token - kokoro first)
    DRAGON_LOCAL_VOICE_URL (default http://127.0.0.1:8124)
    DRAGON_VOICE_PROFILE  (default dragon_deep; 7 profiles on the local server)
    DRAGON_VOICE_MOOD     (optional: smile / confident / whisper ...)
    DRAGON_TTS_ENGINE     (auto | local | edge - default auto = kokoro first, edge backup)
...
# ---------------- config ----------------
PORT = int(os.environ.get("DRAGON_PORT", "5066"))
TOKEN = os.environ.get("DRAGON_TOKEN", "").strip()
VOICE = os.environ.get("DRAGON_VOICE", "en-US-AndrewNeural")
WORKSPACE = Path(os.environ.get("DRAGON_WORKSPACE", str(Path.home() / "dragon_workspace"))).resolve()
WORKSPACE.mkdir(parents=True, exist_ok=True)
TTS_DIR = Path("/tmp/dragon_tts")
...
# Universal Dragon local voice server (kokoro / voice-soul v2)
LOCAL_VOICE_URL = os.environ.get("DRAGON_LOCAL_VOICE_URL", "http://127.0.0.1:8124").rstrip("/")
LOCAL_VOICE_TOKEN = os.environ.get("DRAGON_VOICE_TOKEN", "").strip()
LOCAL_VOICE_PROFILE = os.environ.get("DRAGON_VOICE_PROFILE", "dragon_deep").strip()
LOCAL_VOICE_MOOD = os.environ.get("DRAGON_VOICE_MOOD", "").strip() or None
TTS_ENGINE = os.environ.get("DRAGON_TTS_ENGINE", "auto").strip().lower()  # auto | local | edge
...
logging.basicConfig(level=logging.INFO, format="[DRAGON] %(message)s")
...
SYSTEM_PROMPT = (
    "You are DRAGON, the personal AI voice inside Nova-Pi, a Raspberry Pi 5 homelab "
...
def _tts_local(clean):
    """Universal Dragon local voice server (voice-soul v2 / kokoro). Returns wav fname or None."""
    if not LOCAL_VOICE_TOKEN:
        return None
    try:
        r = requests.post(
            LOCAL_VOICE_URL + "/v2/speak",
            headers={"Authorization": "Bearer " + LOCAL_VOICE_TOKEN,
                     "Content-Type": "application/json"},
            json={"text": clean[:1200], "profile": LOCAL_VOICE_PROFILE,
                  "mood": LOCAL_VOICE_MOOD, "context": "chat",
                  "engine": "auto", "intensity": 0.7},
            timeout=90,
        )
        if r.status_code == 200 and r.content[:4] == b"RIFF":
            fname = uuid.uuid4().hex + ".wav"
            (TTS_DIR / fname).write_bytes(r.content)
            log.info("local voice ok engine=%s ms=%s",
                     r.headers.get("X-Dragon-Voice-Engine", "?"),
                     r.headers.get("X-Dragon-Inference-Ms", "?"))
            return fname
        log.warning("local voice http %s", r.status_code)
    except Exception as e:
        log.warning("local voice failed: %s", e)
    return None

def _tts_edge(clean):
    """edge-tts cloud voice. Returns mp3 fname or None."""
    if not HAS_TTS:
        return None
...
def tts_make(text):
    ...
    if TTS_ENGINE in ("auto", "edge"):
        return _tts_edge(clean)
    return None

def tts_cleanup():
    now = time.time()
    for ext in ("*.mp3", "*.wav"):
        for p in TTS_DIR.glob(ext):
            try:
...
<title>Dragon Voice Login</title><style>
...
<title>Universal Dragon Voice</title><style>
...
<header><div class=dot></div><h1>D R A G O N · V O I C E</h1><span id=brain>...</span></header>
<div id=langbar>
...
const WAKE_WORDS=['hey dragon','hey nova','ok dragon'];
...
bubble('DRAGON online. Say the word, boss.','ai');
</script></body></html>"""
...
def healthz():
    ...
    local_voice = False
    if LOCAL_VOICE_TOKEN:
        try:
            local_voice = bool(requests.get(LOCAL_VOICE_URL + "/health", timeout=2).json().get("ok"))
        except Exception:
            pass
    return jsonify({"ok": True, "brain": brain, "tts": HAS_TTS, "voice": VOICE, "port": PORT,
                    "local_voice": local_voice, "tts_engine": TTS_ENGINE,
                    "voice_profile": LOCAL_VOICE_PROFILE})
...
@require_auth
def api_tts(fname):
    if not re.fullmatch(r"[a-f0-9]{32}\.(mp3|wav)", fname):
        return "nope", 404
    p = TTS_DIR / fname
    if not p.exists():
        return "gone", 404
