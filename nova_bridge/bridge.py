...
VOICE_TOKEN = os.environ.get("NOVA_VOICE_TOKEN", "").strip()
...
AUDIO_DIR = Path(os.environ.get("NOVA_AUDIO_DIR", "/tmp/nova_bridge_audio"))
...
def audio(fname: str, request: Request, authorization: str | None = Header(default=None)):
    identity = _authorize_request(request, authorization)
    _rate_limit(identity)
    if not re.fullmatch(r"[a-f0-9]{32}\.wav", fname):
